...
//  标识上一次 ajax  请求有没回来 ,  没有回来不再进行下一次
/**
 * ajax  提交表单 到后台去验证然后回到前台提示错误
 *  验证通过后 , 再通过  form  自动提交
 * @param form_id  表单 id
 * @param submit_url  提交的 url
 * @param add_url  继续新增的 url
 * @param list_url  返回列表的 url
 * @returns
 */
before_request = 1
function ajax_submit_form(form_id, submit_url, add_url, list_url) {
    ...
    $.ajax({
        ...
        success: function (result) {
            if (200 == result.code) {
                $("#goods_id").val(result.message);
                $("#file-goods-image").removeAttr("disabled");
                layer.confirm('保存成功！', {btn: ['继续新增', '返回列表', '留在本页']},
                    function () {
