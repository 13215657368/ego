...
function get_category(parentId, next, select_id) {
    //给父id赋值
    $("#parentId").val(parentId);
    //用户重新选择顶级分类时，重置下级分类为：请选择选择商品分类，且清空下级分类信息
    ...
    var  url  =  projectName + "/product/category/getByParentId"
...
function    set_parentId(parentId, level) {
    //商品分类值=0，说明是一级菜单，级别为2
    if(0==parentId){
        $("#parentId").val($("#parent_id_1").val());
        $("#level").val(2);
    }else{
        //商品分类1值！=0，说明是二级菜单，级别为3
        $("#parentId").val(parentId);
        $("#level").val(level);
    }
}
...
//获取多级联动的商品分类
function  get_category_goods(id,next,hidder_id,a) {
    $("#"+hidder_id).val();
...
/**
 *  设置隐藏域中 catId 和 extendCatId 的值
 * @param id
 * @param hidden_id
 */
function set_hidden_goods(id, hidden_id) {
    $("#" + hidden_id).val(id);
}