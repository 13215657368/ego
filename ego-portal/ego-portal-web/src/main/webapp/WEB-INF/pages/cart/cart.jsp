...
<div id="doc">
    ...
    <div id="s_bdw">
        <div id="s_bd">
            ...
            <div class="cartlist">
                <form action="${ctx}/order/toPerOrder" method="post">
                    <table width="100%">
                        <thead>
                        <tr>
                            <th>购物车中的商品</th>
                        ...
                        <tbody id="cartBody">
                        <c:forEach var="cart" items="${cartList.goodsVoList}">
                            <tr bgcolor="#fffaf1">
                                <td>
                                ...
                                <td><strong class="red">￥<span
                                        id="qty_marketPrice_${cart.goodsId}">${cart.marketPrice}</span></strong></td>

                                <td>
                                    <input type="hidden" id="qty_goodsNum_${cart.goodsId}"/>
                                    <div class="addinput">
                                        <input type="text" name="qty_item_${cart.goodsId}" value="${cart.goodsNum}"
                                               id="qty_item_${cart.goodsId}"
                                               onblur="setAmount.modify('#qty_item_${cart.goodsId}');"
                                               onfocus="getOldGoodsNum('#qty_goodsNum_${cart.goodsId}', this.value);"
                                               class="stext"/>
                                        <a class="add" onClick="setAmount.add('#qty_item_${cart.goodsId}')"
                                           href="javascript:void(0)"></a>
                                        <a class="reduce" onClick="setAmount.reduce('#qty_item_${cart.goodsId}')"
                                           href="javascript:void(0)"></a>
                                    </div>
                                </td>
                                ...
                                <td>
                                    <a href="javascript:void(0);"
                                       onclick="deleteCart(this, '${cart.goodsId}', '${cart.goodsNum}', '${cart.marketPrice}');"
                                       class="blue">删除
                                    </a>
                                </td>
                            </tr>
                        ...
                        <tr>
                            <td valign="top"><a href="javascript:void(0);" onclick="clearCart(this);"><img
                                    src="${ctx}/images/deleteicon.gif"/>
                                清空购物车</a></td>
                            <td align="right" colspan="5">
                                <p style="margin-top:10px;font-size:14px;">总计金额：<strong
                                        style="font-size:18px;color:#d80000;">￥<span
                                        id="totalPrice">${cartList.totalPrice}</span></strong></p>
                            </td>
                        </tr>
                        <tr>
            ...
            <script type="text/javascript">
                ...
                var setAmount = {
                    ...
                    amount: function (obj, mode) {
                        ...
                        if (this.reg(x)) {
                            ...
                            // 获取单价
                            var goodsIds = obj.split("_");
                            var mpId = "#qty_marketPrice_" + goodsIds[2];
                            var marketPrice = $(mpId).text();
                            ...
                            if (mode) {
                                ...
                                // 单价加总价
                                $("#totalPrice").text((parseFloat(totalPrice) + parseFloat(marketPrice)).toFixed(2));
                                // 同步至redis
                                updateToCart(goodsIds[2], 1, marketPrice, "add");
                            } else {
                                x--;
                                if (x < 1)
                                    return;
                                ...
                                $("#totalPrice").text((parseFloat(totalPrice) - parseFloat(marketPrice)).toFixed(2));
                                // 同步至redis
                                updateToCart(goodsIds[2], -1, marketPrice, "reduce");
                            }
                        } else {
                            alert("请输入正确的数量！");
                        }
                    ...
                    modify: function (obj) {
                        ...
                        // 获取原数量
                        var goodsIds = obj.split("_");
                        var oldNumId = "#qty_goodsNum_" + goodsIds[2];
                        var oldNum = $(oldNumId).val();
                        if (x < this.min || x > this.max || !this.reg(x)) {
                            alert("请输入正确的数量！");
                            $(obj).val(oldNum);
                        } else {
                            // 新数量减去原数量
                            var num = parseInt(x) - parseInt(oldNum);
                            // 获取单价
                            var goodsIds = obj.split("_");
                            var mpId = "#qty_marketPrice_" + goodsIds[2];
                            var marketPrice = $(mpId).text();
                            ...
                            var totalPrice = $("#totalPrice").text();
                            // 单价乘以数量
                            $("#totalPrice").text((parseFloat(marketPrice) * num + parseFloat(totalPrice)).toFixed(2));
                            // 同步至redis
                            updateToCart(goodsIds[2], num, marketPrice);
                        }
                    }
                }

                // 获取商品数量
                function getOldGoodsNum(goodsNumId, val) {
                    $(goodsNumId).val(val);
                }

                // 同步至redis购物车
                function updateToCart(goodsId, goodsNum, marketPrice, flag) {
                    $.ajax({
                        ...
                        success: function (result) {
                            if (200 == result.code) {
                                ...
                                var num = parseInt($("#cartSpan").text()) + goodsNum;
                                $("#cartSpan").text(num);
                            } else {
                ...
                // 删除购物车
                function deleteCart(a, goodsId, goodsNum, marketPrice) {
                    $.ajax({
                        ...
                        success: function (result) {
                            if (200 == result.code) {
                                ...
                                // 获取总件数
                                var num = $("#cartSpan").text();
                                // 重新设置总件数
                                $("#cartSpan").text(parseInt(num) - parseInt(goodsNum));
                                // 计算单个商品的总价格
                                var singleTotal = parseFloat(marketPrice) * parseInt(goodsNum);
                                // 获取总价
                                var totalPrice = $("#totalPrice").text();
                                // 重新计算总价
                                $("#totalPrice").text((parseFloat(totalPrice) - singleTotal).toFixed(2));
                                layer.msg("删除成功！");
                            } else {
                                layer.alert("亲，系统正在升级中，请稍后再试！");
                            }
